...
test:do_execsql_test(
    ...
    [[
        ...
        ANALYZE;
        SELECT DISTINCT "name" FROM "_sql_stat1" JOIN "_space" ON "_sql_stat1"."space_id" = "_space"."id" ORDER BY 1;
    ]], {
        -- <analyze-5.0>
        "T3", "T4"
        -- </analyze-5.0>
...
test:do_execsql_test(
    "analyze-5.1.1",
    string.format([[
            SELECT DISTINCT "name" FROM "%s" JOIN "_space" ON "%s"."space_id" = "_space"."id" ORDER BY 1;
        ]], stat, stat, stat), {
        -- <analyze-5.1>
        "T3", "T4"
        -- </analyze-5.1>
